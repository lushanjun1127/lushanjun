    ...
    if(get_option("iemo_comments_visitor") == 'true') { ?>
      ...
      <?php
        if(isset($_COOKIE["comment_author_" . COOKIEHASH])) {
          $comment_author = sanitize_text_field($_COOKIE["comment_author_" . COOKIEHASH]);
        } else {
          $comment_author = '';
        }

        if(isset($_COOKIE["comment_author_email_" . COOKIEHASH])) {
          $comment_author_email = sanitize_text_field($_COOKIE["comment_author_email_" . COOKIEHASH]);
        } else {
      ...
      <form action="<?php bloginfo('url') ?>/wp-comments-post.php" method="post">
        ...
        <div class="visitor">
          <input id="author" name="author" type="text" value="<?=esc_attr($comment_author)?>" placeholder="昵称*" size="30" maxlength="245" autocomplete="name" required="required">
          <input id="email" name="email" type="text" value="<?=esc_attr($comment_author_email)?>" placeholder="邮箱*" size="30" maxlength="100" aria-describedby="email-notes" autocomplete="email" required="required">
